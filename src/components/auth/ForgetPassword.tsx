...
export default function ForgetPasswordPage() {
  ...
  const { register, watch, handleSubmit } = useForm<ForgetPasswordFormValues>({
    defaultValues: {
      email: '',
    },
  })
  ...
  function submit(values: ForgetPasswordFormValues) {
    if (!canSubmit || isPending) return

    setFormError(null)
    startTransition(async () => {
      const result = await forgotPasswordAction({ email: values.email.trim() })

      if (result.ok) {
        toast.success(result.data.message)
        router.push(
          `/otp?${buildOtpSearchParams(
            result.data.email,
            OTP_FLOW_FORGOT_PASSWORD
          )}`
        )
    ...
    <AuthSplitLayout mainClassName={authMainMobileStickyFooter}>
      <div className={cn(authPageColumn, 'gap-4 sm:gap-6 md:pb-25')}>
        <form
          className={cn(
            authFormFlexGrow,
            'flex-1 justify-between gap-8 sm:gap-10'
          )}
        ...
        >
          <div className={cn(authUpperBlockGrow, 'gap-6 sm:gap-8')}>
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              ...
              <p className="max-w-[461px] text-base leading-6 text-[#6b6e71]">
                Zəhmət olmasa qeydiyyatdan keçdiyiniz email adresini daxil edin,
                sizə təsdiq kodu göndəriləcək.
              </p>
            </div>
          ...
          <div className={cn(authMobileStickySubmit, 'mt-0')}>
            <button
              type="submit"
